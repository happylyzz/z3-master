...
namespace opt {
    ...
    smt::theory_opt& opt_solver::get_optimizer() {
        ...
        else if (typeid(smt::theory_nra&) == typeid(*arith_theory)) {
            // Jia: new added
            return dynamic_cast<smt::theory_nra&>(*arith_theory); 
        }
        else {
            UNREACHABLE();
            return dynamic_cast<smt::theory_mi_arith&>(*arith_theory); 
    ...
    */
    ...
    expr_ref opt_solver::mk_ge(unsigned var, inf_eps const& _val) {
        ...
        if (typeid(smt::theory_nra) == typeid(opt)) {
            smt::theory_nra& th = dynamic_cast<smt::theory_nra&>(opt); 
            SASSERT(val.is_finite());
            return th.mk_ge(m_fm, v, val.get_numeral());            
        }//add
