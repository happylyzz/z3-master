...
//若is_int为1则输出整数类型的1，否则输出有理数类型的1
...
//将给定表达式t内部化到上下文中，若force=true则说明内部化成功
...
//将等式约束内部化    
    void internalize_eq(theory_var v1, theory_var v2) {  
        app_ref term(m.mk_const("eq", a.mk_real()), m);
...
//给布尔变量断言赋值（每当v被赋值为true时，就会调用该函数）
...
//为l1和l2,l3添加相关约束，若l1,l2,l3之间有关联也需要添加相应的表述。
...
//假设变量之间的等式相对于给定的表相同，表是被变量值索引的哈希表。
    bool assume_eqs() {        
...
//判断m_assume_eq_candidates中是否有相等项
...
//根据is_sat的情况判断continue或give_up，最终再对事件处理器进行检查
...
//如果lower_bound为true则添加term>=k，否则添加term<k,如果term要求为整数还需要调整
...
//判断核
    ...
    literal is_bound_implied(lp::lconstraint_kind k, rational const& value, api_bound const& b) const {
        // Jia: basic assertion of implied bounds
        ...
        // Jia: If not implked, return null
        return null_literal;
    }
    ...
    void propagate_bound(bool_var bv, bool is_true, api_bound& b) {
        // Jia: construct m_core for implied bound
    ...
    bool get_bound(api_bound const& b, inf_rational& r, bool is_lub) {
        ...
        for (auto const mono : term) {
            ...
            if (mono.coeff().is_neg() == is_lub) {
                ...
                if (is_strict) {
                    r += inf_rational(rational::zero(), mono.coeff().is_pos());//如果是严格的开区间需要加一个微量epsilon
                }
            }
            else {
